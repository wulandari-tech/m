...
exports.updateUserProfile = async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        if (req.file) {
            fs.unlink(req.file.path, (err) => {
                if (err) console.error("Error deleting uploaded file during validation fail (updateUserProfile):", err);
            });
        }
    ...
    try {
            ...
            if (user.profilePicture && user.profilePicture !== '/images/default-avatar.png' && !user.profilePicture.startsWith('http')) {
                ...
                if (fs.existsSync(oldPicPath)) {
                    fs.unlink(oldPicPath, (err) => {
                        if (err) console.error("Error deleting old profile picture (updateUserProfile):", err);
                    });
                }
            }
        ...
        if (req.file) {
            fs.unlink(req.file.path, (err) => {
                if (err) console.error("Error deleting uploaded file during save fail (updateUserProfile):", err);
            });
        }
...
exports.updateQrisSettings = async (req, res) => {
    ...
    try {
        ...
        if (user.role !== 'seller' && user.role !== 'admin') {
            req.flash('error_msg', 'Hanya seller atau admin yang dapat mengatur QRIS');
...
exports.updateApiSettings = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
         ...
         if (user.role !== 'seller' && user.role !== 'admin') {
            req.flash('error_msg', 'Hanya seller atau admin yang dapat mengatur API');
            return res.redirect('/dashboard');
        }
...
exports.getMySourceCodesPage = async (req, res) => {
    try {
        ...
        res.render('user/my_sc_list', {
            titlePage: 'Produk Saya',
            mySourceCodes,
            breadcrumbs: [{ name: 'Dashboard', url: '/dashboard' }, { name: 'Produk Saya', active: true }]
        });
    } catch (error) {
        console.error("Error in getMySourceCodesPage:", error);
...
exports.getSellerStorePage = async (req, res) => {
    try {
        const seller = await User.findById(req.params.sellerId).select('-password -qrisBaseCode -qrisMerchantId -qrisApiKey -pterodactylClientApiKey -pterodactylAppApiKey -resetPasswordToken -resetPasswordExpires -pterodactylDefaultLocationId -pterodactylDefaultNestId -pterodactylDefaultEggId');
        ...
        console.error("Error in getSellerStorePage:", error);
